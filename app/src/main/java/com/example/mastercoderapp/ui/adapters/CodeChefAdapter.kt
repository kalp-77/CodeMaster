...
class CodeChefAdapter(private val context: CodeChefFragment, private val articles: List<ContestDetailsItem>) : RecyclerView.Adapter<CodeChefAdapter.ViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(context.requireContext()).inflate(R.layout.code_chef_layout, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.end.text = "At: " + article.startTime.subSequence(10,16)
        if((article.duration.toInt()%86400) / 3600 != 0){
            holder.duration.text = "Duration: " + (article.duration.toInt()%86400) / 3600 + " hrs"
        }
        else{
            holder.duration.text = "Duration: No"
        }
